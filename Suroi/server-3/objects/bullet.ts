...
export class Bullet extends BaseBullet {
  ...
  constructor(game: Game, source: Weapon, shooter: GameObject, options: ServerBulletOptions) {
    const reference =
      source instanceof GunItem && source.definition.isDual
  ...
  update(): DamageRecord[] {
    const lineRect = RectangleHitbox.fromLine(
      this.position,
      Vec.add(this.position, Vec.scale(this.velocity, GameConstants.msPerTick))
    );
    ...
    for (const collision of collisions) {
      ...
      if (object instanceof Obstacle) {
        ...
        if (!object.definition.noCollisions) {
          ...
          if (object.definition.reflectBullets && this.reflectionCount < 10) {
