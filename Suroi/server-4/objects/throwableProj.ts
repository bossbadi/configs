  ...
  update(): void {
    ...
    const canFlyOver = (obstacle: Obstacle): boolean => {
      return flyoverCondMap[
        obstacle.door?.isOpen === false ? FlyoverPref.Never : obstacle.definition.allowFlyover ?? FlyoverPref.Sometimes
      ];
    };
      ...
      const handleRectangle = (hitbox: RectangleHitbox): void => {
        ...
        if (collision) {
          this.velocity = Vec.add(
            this._velocity,
            Vec.scale(Vec.project(this._velocity, Vec.scale(collision.dir, 1)), -1.5)
          );
        ...
        case HitboxType.Group: {
          for (const target of hitbox.hitboxes) {
            if (target.collidesWith(this.hitbox)) {
              target instanceof CircleHitbox ? handleCircle(target) : handleRectangle(target);
            }
          }
          break;
        }
